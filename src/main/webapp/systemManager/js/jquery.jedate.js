...
;(function (root, factory) {
    ...
    var jet = {}, doc = document, ymdMacth = /\w+|d+/g, parseInt = function (n) {
    ...
    jet.initDates = function (num, format) {
        ...
        var years = newDate.getFullYear(), months = newDate.getMonth() + 1, days = newDate.getDate(),
    ...
    jet.addDateTime = function (time, num, type, format) {
        ...
        var tarr = jet.IsNum(time) ? nocharDate.match(ymdMacth) : time.match(ymdMacth), date = new Date(),
            tm0 = parseInt(tarr[0]), tm1 = tarr[1] == undefined ? date.getMonth() + 1 : parseInt(tarr[1]),
    ...
    jedfn.onlyYMevents = function (tmsArr, opts) {
        var that = this, boxCell = $(jet.boxCell);
        var ymVal, ymPre = boxCell.find(".jedateym .ymprev"), ymNext = boxCell.find(".jedateym .ymnext"),
    ...
    jedfn.chooseDays = function (opts) {
        ...
        boxCell.find(".jedaul li").on("click", function (ev) {
            ...
            var aty = parseInt(_that.attr("year")), atm = parseFloat(_that.attr("month")),
        ...
        if (opts.festival) {
            ...
            boxCell.find(".jedaul li").on("mouseover", function () {
                var _this = $(this), aty = parseInt(_this.attr("year")), atm = parseFloat(_this.attr("month")),
    ...
    jedfn.chooseYM = function (opts) {
        var that = this, boxCell = $(jet.boxCell);
        var jetopym = boxCell.find(".jedatetopym"), jedateyy = boxCell.find(".jedateyy"),
            jedatemm = boxCell.find(".jedatemm"), jedateyear = boxCell.find(".jedateyy .jedateyear"),
            jedatemonth = boxCell.find(".jedatemm .jedatemonth"), mchri = boxCell.find(".jedateymchri"),
    ...
    jedfn.ymPremNextEvents = function (opts) {
        var that = this, elemCell = that.valCell, boxCell = $(jet.boxCell);
        var newDate = new Date(), isYY = (jet.checkFormat(jet.format) == "YYYY"),
    ...
    jedfn.events = function (tmsArr, opts) {
        ...
        if (!isYYMM) {
            ...
            var hs = boxCell.find(".jedateprophours"), ms = boxCell.find(".jedatepropminutes"),
        ...
        boxCell.find(".jedatehms input").on("keyup", function () {
            var _this = $(this), thatval = _this.val(), hmsVal = parseInt(_this.attr("numval")),
        ...
        boxCell.find(".jedatebot .jedateok").on("click", function (ev) {
            ev.stopPropagation();
            var isValtext = (elemCell.val() || elemCell.text()) != "", isYYYY = jet.checkFormat(jet.format) == "YYYY",
                okVal = "",
                //获取时分秒的数组
                eachhmsem = function () {
                    var hmsArr = [];
