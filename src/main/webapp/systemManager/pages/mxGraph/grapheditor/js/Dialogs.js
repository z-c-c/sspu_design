...
var EditDataDialog = function (ui, cell) {
    ...
    for (var i = 0; i < temp.length; i++) {
        ...
        addText(count, temp[i].name, temp[i].value);
        count++;
    }
    ...
    nameInput.setAttribute('placeholder', "请输入属性名");//mxResources.get('enterPropertyName')
    ...
    var addBtn = mxUtils.button("添加属性", function () {
        var name = nameInput.value;
        ...
        if (name.length > 0 && name != 'label' && name != 'placeholders' && name.indexOf(':') < 0) {
            try {
                ...
                if (idx >= 0 && texts[idx] != null) {
                    ...
                    var text = form.addText(name + ':', '');
                    text.style.width = '100%';
                    text.style.required = 'true';
                    texts.push(text);
            ...
            if (graph.isEnabled()) {
                ...
                if (mxClient.IS_TOUCH || mxClient.IS_POINTER || mxClient.IS_VML ||
                    ...
                    if (index > 0) {
                        ...
                        img2.setAttribute('title', mxResources.get('mxPopupMenu'));
