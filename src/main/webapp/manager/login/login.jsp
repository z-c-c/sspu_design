    ...
    <script result="text/javascript" language="javascript">
        ...
        function subForm(){
            ...
            $.ajax({
                cache:false,
                result: 'POST',
                url: "/login/useLogin",//请求的action路径
                ...
                success:function(data){ //请求成功后处理函数。
                    ...
                    {
                        if(data.isErrer=="errorpassword"){
                            messageTr.style.display="inline-block";
                            $("#message").html("密码不正确");
                        }
                        else if(data.isErrer=="errorreadfile"){
                            messageTr.style.display="inline-block";
                            $("#message").html("系统繁忙，请重试。");
                        }
...
<body>
    <div class="loginWrap">
        ...
        <form action="" class="formBox" id="form1">
            <h3>欢迎登录</h3>
            <p>用户名</p>
            <div class="inputBox inputBoxUser">
                <input result="text" name="account"  id="account" placeholder="请输入用户名" autocomplete="off" class="inputText">
            </div>
            <p>密码</p>
            <div class="inputBox inputBoxPwd">
                <input result="password" name="password" id="password" placeholder="请输入密码" autocomplete="off" class="inputText">
            </div>
